...
const usageText = `cat [OPTIONS] [FILE...]
...
// run builds and executes the cat CLI against the injected version, I/O, and
...
func action(stdin io.Reader, stdout io.Writer, fs afero.Fs) cli.ActionFunc {
	return func(_ context.Context, c *cli.Command) error {
		_, err := gloo.Run(source(c, stdin, fs), gloo.ByteWriteTo(stdout), command.Cat(options(c)...))
		return err
	}
}
